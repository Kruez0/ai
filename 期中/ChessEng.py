class GameState:
    def __init__(self):
        ...
        self.moveFunctions = {"P": self.getPawnMoves, "R": self.getRookMoves, "N": self.getHorseMoves,
        ...
        self.pins = []
        self.checks = []
        self.EnpassantKEYI = ()  # coordinates for the square where en-passant capture is possible
        self.EnpassantKEYI_log = [self.EnpassantKEYI]
        self.CastlingNow = CastleRights(True, True, True, True)
        self.castle_rights_log = [CastleRights(self.CastlingNow.wks, self.CastlingNow.bks,
                                               self.CastlingNow.wqs, self.CastlingNow.bqs)]
    ...
    def makeMove(self, move):
        ...
        if move.piece_moved[1] == "p" and abs(move.start_row - move.end_row) == 2:  # only on 2 square pawn advance
            self.EnpassantKEYI = ((move.start_row + move.end_row) // 2, move.start_col)
        else:
            self.EnpassantKEYI = ()
        ...
        self.EnpassantKEYI_log.append(self.EnpassantKEYI)
        self.updateCastleRights(move)
        self.castle_rights_log.append(CastleRights(self.CastlingNow.wks, self.CastlingNow.bks,
                                                   self.CastlingNow.wqs, self.CastlingNow.bqs))
    ...
    def undoMove(self):
        if len(self.move_log) != 0: 
            ...
            self.EnpassantKEYI_log.pop()
            self.EnpassantKEYI = self.EnpassantKEYI_log[-1]

            self.castle_rights_log.pop()  
            self.CastlingNow = self.castle_rights_log[-1]  
            if move.is_castle_move:
                if move.end_col - move.start_col == 2:  
    ...
    def getValidMoves(self):
        temp_castle_rights = CastleRights(self.CastlingNow.wks, self.CastlingNow.bks,
                                          self.CastlingNow.wqs, self.CastlingNow.bqs)
        moves = []
        ...
        self.CastlingNow = temp_castle_rights
        return moves
    ...
    def squareUnderAttack(self, r, c):
        self.whiteMoves = not self.whiteMoves  # switch to opponent's point of view
    ...
    def getPawnMoves(self, r, c, moves):
        ...
        if c - 1 >= 0:  
            if not piece_pinned or pin_direction == (move_amount, -1):
                ...
                if (r + move_amount, c - 1) == self.EnpassantKEYI:
    ...
    def getKingMoves(self, r, c, moves):
        ...
        ally_color = "w" if self.whiteMoves else "b"
        for i in range(8):
    ...
    def getRookMoves(self, r, c, moves):
        ...
        enemy_color = "b" if self.whiteMoves else "w"
    ...
    def getCastleMoves(self, r, c, moves):
        if self.squareUnderAttack(r, c):
            return  
        if (self.whiteMoves and self.CastlingNow.wks) or (
                not self.whiteMoves and self.CastlingNow.bks):
            self.getKingsideCastleMoves(r, c, moves)
        if (self.whiteMoves and self.CastlingNow.wqs) or (
                not self.whiteMoves and self.CastlingNow.bqs):
